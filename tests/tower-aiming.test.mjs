...
test("kule dönüş hızı önceki 36 rad/sn değerinin iki katıdır", () => {
  assert.equal(TOWER_TURN_RATE_RADIANS_PER_SECOND, 72);
});

test("kule hedefe doğru sınırlı adımla döner ve hedefi geçmez", () => {
  assert.equal(rotateTowerTowards(0, Math.PI, 1 / 144), 0.5);
