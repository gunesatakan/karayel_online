 ...
 * cunku isci binaya hic dokunmuyordu. Savas kuleleri ise kapali kaliyor.
 *
...
test("işçi yükleme yaptığı yapıya girebilir", () => {
  ...
  // Ayni isci teslim asamasindayken giremez: aciklik yuklemeye bagli.
  const teslim = isci(room, { phase: "deliver", targetTowerId: kule.id });
  assert.equal(room.isWorkerCellOpen(c.col, c.row, teslim), false, "teslimde de acildi");
});
